/*
 * jtrdll_linux_x86_64.c
 * Implements the jtrdll API as a wrapper around hashcat for Linux x86_64 / ARM64.
 * Builds to: jtrdll_linux-x86_64.so
 *
 ...
 * Configuration
 * ============================================================ */
#define HASHCAT_BIN_DEFAULT  "/usr/bin/hashcat"
#define HASHCAT_BIN_ALT      "/usr/local/bin/hashcat"
#define HASHCAT_RULES        "/usr/share/hashcat/rules"
#define HASHCAT_RULES_ALT    "/usr/local/share/hashcat/rules"

/* Find the first accessible hashcat binary */
static const char *find_hashcat_bin(void)
{
    if (access(HASHCAT_BIN_DEFAULT, X_OK) == 0) return HASHCAT_BIN_DEFAULT;
    if (access(HASHCAT_BIN_ALT,     X_OK) == 0) return HASHCAT_BIN_ALT;
    return "hashcat"; /* fall back to PATH */
}

/* Find the first accessible hashcat rules directory */
static const char *find_rules_dir(void)
{
    struct stat st;
    if (stat(HASHCAT_RULES,     &st) == 0 && S_ISDIR(st.st_mode)) return HASHCAT_RULES;
    if (stat(HASHCAT_RULES_ALT, &st) == 0 && S_ISDIR(st.st_mode)) return HASHCAT_RULES_ALT;
    return HASHCAT_RULES; /* best-effort default */
}
...
static const char *resolve_hashcat_bin(void)
{
    ...
    const char *bin = find_hashcat_bin();
    snprintf(resolved, sizeof(resolved), "%s", bin);
    return resolved;
}
...
static void add_hashcat_rule_if_any(const char *jtr_rules)
{
    ...
    /* LC7/JtR passes either a rule set name (best64) or a concrete .rule file path */
    ...
    if (strcasecmp(jtr_rules, "best64") == 0 || strcasecmp(jtr_rules, "Best64") == 0) {
        ...
        snprintf(rules_path, PATH_MAX, "%s/best64.rule", find_rules_dir());
...
static char   s_hc_potfile_read[PATH_MAX] = "";
static char   s_jtr_potfile[PATH_MAX] = "";
...
/* ============================================================
 * Execute hashcat (posix_spawn — safe in multi-threaded processes)
...
static int run_hashcat(struct JTRDLL_HOOKS *hooks)
{
    ...
    /* posix_spawn — safe in multi-threaded processes on Linux */
    ...
    /* No macOS-specific spawn attributes (posix_spawnattr_setbinpref_np is macOS-only) */
...
int jtrdll_main(int argc, char **argv, struct JTRDLL_HOOKS *hooks)
{
        ...
        if (strcmp(hashcat_bin, "hashcat") != 0 && access(hashcat_bin, X_OK) != 0) {
            if (hooks && hooks->stderr_hook)
                hooks->stderr_hook(hooks->ctx,
                    "hashcat not found (checked /usr/bin/hashcat, /usr/local/bin/hashcat, PATH)");
            return 1;
        }
    ...
    /* Multi-node: only node 1 runs hashcat; others skip. */
